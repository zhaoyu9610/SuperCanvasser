...
class Campaign(models.Model):
    ...
    def dict(self):
        # return {'id': self.id, 'name': self.name, 'talking_points': json.loads(self.talking_points),
        #         'start': self.start, 'finish': self.finish, 'median': self.median, 'duration': self.duration,
        #         'average': self.average, 'sd': self.sd, 'questions': json.loads(self.questions),
        #         'manager': [a.dict() for a in self.managers.all()],
        #         'canvassers': [a.dict() for a in self.canvassers.all()],
        #         'locations': [a.dict() for a in self.locations.all()],
